...
class UserRepository(private val baseUrl: String) : IUserRepository {
    ...
    override suspend fun update(id: String, newUser: User): Boolean = suspendTransaction {
        val userDAO = UserDAO.find { UserTable.id eq UUID.fromString(id) }
            .limit(1).firstOrNull()
        if (userDAO != null) {
            ...
            userDAO.updatedAt = kotlinx.datetime.Clock.System.now()
            true
        } else false
    }

    override suspend fun delete(id: String): Boolean = suspendTransaction {
        val rowsDeleted = UserTable.deleteWhere { UserTable.id eq UUID.fromString(id) }
        rowsDeleted >= 1
    }
}